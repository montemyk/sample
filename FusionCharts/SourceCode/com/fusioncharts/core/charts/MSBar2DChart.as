...
class com.fusioncharts.core.charts.MSBar2DChart extends SingleYAxis2DHorizontalChart 
{
	...
	*/
	function MSBar2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String)
	{
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				if (this.dataset [i].data [j].isDefined)
				{
					...
					//Create bar instance
					barIns = new Bar2D (barMC, this.dataset [i].data [j].w, this.dataset [i].data [j].h, this.params.plotBorderColor, this.params.plotBorderAlpha, this.params.plotBorderThickness, this.dataset [i].data [j].color, this.dataset [i].data [j].alpha, this.dataset [i].data [j].ratio, this.params.plotFillAngle, false, this.dataset [i].data [j].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
					//Draw the bar
					barIns.draw ();
