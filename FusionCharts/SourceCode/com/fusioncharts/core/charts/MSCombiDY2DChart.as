...
class com.fusioncharts.core.charts.MSCombiDY2DChart extends DualYAxis2DChart 
{
	...
	*/
	function MSCombiDY2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String)
	{
			...
			if (this.dataset [i].renderAs == "COLUMN")
			{
				for (j = 1; j <= this.num; j ++)
				{
					...
					if (this.dataset [i].data [j].isDefined)
					{
						...
						colIns = new Column2D (colMC, this.dataset [i].data [j].w, this.dataset [i].data [j].h, this.dataset [i].plotBorderColor, this.dataset [i].plotBorderAlpha, this.dataset [i].plotBorderThickness, colors, alphas, ratios, this.params.plotFillAngle, false, this.dataset [i].data [j].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
						//Draw the column
						colIns.draw ();
