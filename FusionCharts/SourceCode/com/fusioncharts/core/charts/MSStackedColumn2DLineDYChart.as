...
class com.fusioncharts.core.charts.MSStackedColumn2DLineDYChart extends DualYAxis2DChart 
{
	...
	*/
	function MSStackedColumn2DLineDYChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String)
	{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				for (k = 1; k <= this.dataset [j].numSDS; k ++)
				{
					...
					if (this.dataset [j].dataset [k].data [i].isDefined)
					{
						...
						colIns = new Column2D (colMC, this.dataset [j].dataset [k].data [i].w, this.dataset [j].dataset [k].data [i].h, this.params.plotBorderColor, this.params.plotBorderAlpha, this.params.plotBorderThickness, this.dataset [j].dataset [k].data [i].color, this.dataset [j].dataset [k].data [i].alpha, this.dataset [j].dataset [k].data [i].ratio, this.params.plotFillAngle, false, this.dataset [j].dataset [k].data [i].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
						//Draw the column
						colIns.draw ();
						//Set y position
