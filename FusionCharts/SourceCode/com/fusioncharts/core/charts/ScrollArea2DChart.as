...
*
* ScrollArea2DChart chart extends the ScrollSingleYAxis2DVerticalChart class to render a
* Scroll 2D Area Chart.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.ScrollArea2DChart extends ScrollSingleYAxis2DVerticalChart 
{
	...
	*/
	function ScrollArea2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String)
	{
		...
		this.log ("Chart Type", "Scroll 2D Area Chart", Logger.LEVEL.INFO);
	...
	*/
	private function calculatePoints ()
	{
		...
		//If the user has not specified numVisiblePlot, we need to assume a value.
		if(this.params.numVisiblePlot ==undefined || this.params.numVisiblePlot=="" || isNaN(this.params.numVisiblePlot)){
			//If the total number of display points is more than our constant value,
			//we show only n/2 data points in view area.
		...
		} else if (this.config.yMax <= 0 && this.config.yMin < 0)
		{
			...
			this.config.basePlanePos = this.getAxisPosition (this.config.yMax, this.config.yMax, this.config.yMin, 0, this.elements.canvas.h, true, 0);
		} 
		else 
		{
		...
		var contentWidth:Number = interPointWidth*(this.num-1);
			...
			if (this.dataset [i].drawAnchors)
			{
				for (j = 1; j <= this.num; j ++)
				{
					...
					if (this.dataset [i].data [j].isDefined)
					{
						...
						anchorMC = this.drawCanvasMC.createEmptyMovieClip ("Anchor_" + i, depth);
