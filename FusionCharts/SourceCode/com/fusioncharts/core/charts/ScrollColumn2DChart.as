...
*
* ScrollColumn2DChart chart extends the ScrollSingleYAxis2DVerticalChart class to render a
* Scroll 2D Column Chart.
*/
//Import Chart class
...
class com.fusioncharts.core.charts.ScrollColumn2DChart extends ScrollSingleYAxis2DVerticalChart 
{
	...
	*/
	function ScrollColumn2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String)
	{
		...
		this.log ("Chart Type", "Scroll 2D Column Chart", Logger.LEVEL.INFO);
		//List Chart Objects and set them
		_arrObjects = new Array ("BACKGROUND", "CANVAS", "CAPTION", "SUBCAPTION", "YAXISNAME", "XAXISNAME", "DIVLINES", "YAXISVALUES", "HGRID", "DATALABELS", "DATAVALUES", "SCROLLPANE", "TRENDLINES", "TRENDVALUES", "DATAPLOT", "TOOLTIP", "VLINES", "LEGEND");
	...
	*/
	private function setStyleDefaults () : Void 
	{
		...
		//-----------------------------------------------------------------//
		//Default Animation object for ScrollPane (if required)
		//-----------------------------------------------------------------//
		if (this.params.defaultAnimation) {
			//Alpha effect for ScrollPane
			var scrollPaneAnim = new StyleObject();
			scrollPaneAnim.name = "_SdScrollPaneAnim";
			scrollPaneAnim.param = "_alpha";
			scrollPaneAnim.easing = "regular";
			scrollPaneAnim.wait = 0;
			scrollPaneAnim.start = 0;
			scrollPaneAnim.duration = 0.5;
			//Over-ride
			this.styleM.overrideStyle(this.objects.SCROLLPANE, scrollPaneAnim, this.styleM.TYPE.ANIMATION, "_alpha");
			delete scrollPaneAnim;
		}
		//-----------------------------------------------------------------//
	...
	*/
	private function calculatePoints ()
	{
		...
		if(this.params.numVisiblePlot ==undefined || this.params.numVisiblePlot=="" || isNaN(this.params.numVisiblePlot)){
			//If the total number of data points specified in XML is more than our required
			//constant (MIN_SCROLLPOINTS), we show half the number of columns by default.
			//Rest would be available on scrolling.
			if((this.num*this.numDS) > MIN_SCROLLPOINTS) {
				this.params.numVisiblePlot = Math.ceil(this.num*this.numDS/2);
			} else {
		...
		//If no scroller is required reduce the space alloted for the ScrollPane
		...
		if (this.config.yMax >= 0 && this.config.yMin < 0)
		{
			...
			this.config.basePlanePos = this.getAxisPosition (0, this.config.yMax, this.config.yMin, 0, this.elements.canvas.h, true, 0);
		} else 
		{
			//No negative numbers - so set basePlanePos as yMin value
			this.config.basePlanePos = this.getAxisPosition (this.config.yMin, this.config.yMax, this.config.yMin, 0, this.elements.canvas.h, true, 0);
		}
		...
		if(this.config.scrollRequired) { 
			//Now, calculate the spacing on canvas
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				dataEndY = this.getAxisPosition (this.dataset [j].data [i].value, this.config.yMax, this.config.yMin, 0, this.elements.canvas.h, true, 0);
		...
		this.drawCanvasMC.lineTo((this.categories[this.num].x+interBlockSpace+(columnWidth * this.numDS * 0.5)), 0);
	...
	*/
	private function drawHeaders ()
	{
		...
		if (this.params.xAxisName != "")
		{
			...
			if (this.params.animation)
			{
				this.styleM.applyAnimation (xAxisNameObj.tf, this.objects.XAXISNAME, this.macro, this.elements.canvas.x + (this.elements.canvas.w / 2) - (this.elements.subCaption.w / 2) , 0, this.elements.canvas.h + this.config.labelAreaHeight + this.params.xAxisNamePadding, 0, 100, null, null, null);
			}
			//Apply filters
		...
		for (i = 1; i <= this.numDS; i ++)
		{
			for (j = 1; j <= this.num; j ++)
			{
				...
				if (this.dataset [i].data [j].isDefined)
				{
					...
					colMC = this.drawCanvasMC.createEmptyMovieClip ("Column_" + i + "_" + j, depth);
