...
class com.fusioncharts.core.charts.StackedBar2DChart extends SingleYAxis2DHorizontalChart 
{
	...
	*/
	function StackedBar2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String)
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				if (this.dataset [j].data [i].isDefined)
				{
					...
					//Create bar instance
					barIns = new Bar2D (barMC, this.dataset [j].data [i].w, this.dataset [j].data [i].h, this.params.plotBorderColor, this.params.plotBorderAlpha, this.params.plotBorderThickness, this.dataset [j].data [i].color, this.dataset [j].data [i].alpha, this.dataset [j].data [i].ratio, this.params.plotFillAngle, false, this.dataset [j].data [i].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
					//Draw the bar
					barIns.draw ();
					//Set x position
