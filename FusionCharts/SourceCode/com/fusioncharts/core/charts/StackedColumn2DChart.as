...
class com.fusioncharts.core.charts.StackedColumn2DChart extends SingleYAxis2DVerticalChart
{
	...
	*/
	function StackedColumn2DChart (targetMC : MovieClip, depth : Number, width : Number, height : Number, x : Number, y : Number, debugMode : Boolean, lang : String)
	{
		...
		for (i = 1; i <= this.num; i ++)
		{
			...
			for (j = 1; j <= this.numDS; j ++)
			{
				...
				if (this.dataset [j].data [i].isDefined)
				{
					...
					colIns = new Column2D (colMC, this.dataset [j].data [i].w, this.dataset [j].data [i].h, this.params.plotBorderColor, this.params.plotBorderAlpha, this.params.plotBorderThickness, this.dataset [j].data [i].color, this.dataset [j].data [i].alpha, this.dataset [j].data [i].ratio, this.params.plotFillAngle, false, this.dataset [j].data [i].dashed, this.params.plotBorderDashLen, this.params.plotBorderDashGap);
					//Draw the column
					colIns.draw ();
					//Set y position
